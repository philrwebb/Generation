...
const getRefDataAssociationsForClass = (c: Class): Association[] => {
...
const getCollectionAssociationsForClass = (c: Class): Association[] => {
  return model.associations.filter(
    (a) =>
...
const doSchemaCollectionAssociations = (
  ...
  if (c.parent?.name !== undefined) {
    schemaContent += doSchemaCollectionAssociations(
      c.parent,
      getCollectionAssociationsForClass(c.parent),
      schemaContent,
    );
  }
...
for (const c of model.classes) {
  const refDataAssociations = getRefDataAssociationsForClass(c);
  // find 1 to * (e.g. class with collections)
  const collectionAssociations = getCollectionAssociationsForClass(c);
  let filePath;
  if (!c.isAbstract) {
    ...
    fs.writeFileSync(filePath + '/' + c.name + '.ts', schemaContent, 'utf8');
  }
